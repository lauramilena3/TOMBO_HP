...
# Read counts
	...
	for index,row in sample_sheet.iterrows():
		...
		if len(ALTERNATIVE_MODELS)>0:
			inputs.extend(expand(dirs_dict["TOMBO"] + "/{genome}_{sample}_{mapping}.tombo_alternative_{model}/{genome}_{sample}_{mapping}.tombo_alternative_{model}.{model}.tombo.stats", sample=row_sample, model=ALTERNATIVE_MODELS, genome=row_genome)),
...
rule resquiggle_tombo:
	...
	shell:
		"""
		tombo resquiggle --dna {input.single_data} {input.genome} --processes {threads} --overwrite --ignore-read-locks --corrected-group "{wildcards.genome}_default"
...
rule resquiggle_tombo_loose:
	...
	shell:
		"""
		tombo resquiggle --dna {input.single_data} {input.genome} --processes {threads} --overwrite --ignore-read-locks --corrected-group "{wildcards.genome}_loose"
		touch {output.resquiggled}
		"""
...
rule tombo_alternative:
	...
	shell:
		...
		mkdir {params.tombo_results_dir}
		cd {params.tombo_results_dir}
		tombo detect_modifications alternative_model --alternate-bases {wildcards.model} --fast5-basedirs {input.sample} --statistics-file-basename {params.name} --per-read-statistics-basename {params.readstats} --processes {threads} --corrected-group "{wildcards.genome}_{wildcards.mapping}"
		tombo text_output browser_files --fast5-basedirs {input.sample} --statistics-filename {output.stats} --genome-fasta {input.genome} --browser-file-basename {params.name} --file-types coverage valid_coverage fraction dampened_fraction signal signal_sd --corrected-group "{wildcards.genome}_{wildcards.mapping}"
		tombo text_output signif_sequence_context --statistics-filename {output.stats} --genome-fasta {input.genome} --num-regions 10000 --num-bases 10 --sequences-filename {output.significant} --corrected-group "{wildcards.genome}_{wildcards.mapping}"
		"""
