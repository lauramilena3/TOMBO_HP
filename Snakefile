...
# Read counts
	...
	for index,row in sample_sheet.iterrows():
		...
		if len(ALTERNATIVE_MODELS)>0:
			inputs.extend(expand(dirs_dict["TOMBO"] + "/{genome}_{sample}.tombo_alternative_{model}/{genome}_{sample}.tombo_alternative_{model}.{model}.tombo.stats", sample=row_sample, model=ALTERNATIVE_MODELS, genome=row_genome)),
	return inputs
