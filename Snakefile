...
dir_names = ["rules", "../envs", OUTPUT_DIR + "/db", OUTPUT_DIR + "/tools", OUTPUT_DIR + "/01_GUPPY", OUTPUT_DIR + "/01_GUPPY/02_DEMULTIPLEXED", OUTPUT_DIR + "/01_GUPPY/02_BASECALLED", OUTPUT_DIR + "/01_GUPPY/02_FAST5_SINGLE", OUTPUT_DIR + "/GENOMES", OUTPUT_DIR + "/02_TOMBO"]
...
rule tombo:
	...
	shell:
		"""
		tombo preprocess annotate_raw_with_fastqs --fast5-basedir {input.sample} --fastq-filenames {wildcards.sample}.fastq --overwrite --quiet
		tombo preprocess annotate_raw_with_fastqs --fast5-basedir {input.control} --fastq-filenames {wildcards.control}.fastq --overwrite --quiet
